...
{
    public partial class Requests_ViewUserControl : UserControl
    {
        ...
        public Requests_ViewUserControl()
        {
            InitializeComponent();

            this.Load += Requests_ViewForm_Load;
            this.Disposed += Requests_ViewForm_Disposed;
        ...
        private void Requests_ViewForm_Disposed(object? sender, EventArgs e)
        {
            this.Load -= Requests_ViewForm_Load;
            this.Disposed -= Requests_ViewForm_Disposed;
            button_update.Click -= Button_update_Click;
        ...
        {
            new Requests_AddEditForm(DBT_Requests.GetById((int)dataGridView.CurrentCell.OwningRow.Cells[0].Value)).ShowDialog();
            UpdateTable();
        }

        private void Button_create_Click(object? sender, EventArgs e)
        {
            new Requests_AddEditForm().ShowDialog();
            UpdateTable();
        }
        ...
        private void Button_remove_Click(object? sender, EventArgs e)
        {
            ...
            int res = DBT_Requests.Remove((int)dataGridView.CurrentCell.OwningRow.Cells[0].Value);
            if (res == -1) MessageBox.Show("Ошибка удаления! Запись используеться!");
        ...
        private void Requests_ViewForm_Load(object sender, EventArgs e) => UpdateTable();
